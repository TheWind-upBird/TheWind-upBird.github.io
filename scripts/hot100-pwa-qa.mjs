...
for(const marker of ['wa2StatusStrip','wa2-winter-scene.svg','WINTER LOG','NOW PLAYING','TRACK LIST','wa2CornerRail','--wa2-serif'])if(!polish.includes(marker))fail(`wa2-polish-pass.js missing polish marker: ${marker}`);
...
if(!sw.includes("CACHE='hot100-shell-v9'"))fail('service worker cache version must be v9');
if(!sw.includes('if(sameOrigin)')||!sw.includes('fetch(req).then'))fail('same-origin assets must use network-first refresh behavior');

console.log(`Adaptive/PWA QA passed: ${requiredCache.length} assets covered; static tools, three themes, WA2 polish, and network-first updates included.`);
