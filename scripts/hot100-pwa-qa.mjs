...
for(const [name,src] of [['mobile-install-fix.js',install],['mobile-tools-drawer.js',drawer],['theme-pass.js',theme],['wa2-design-pass.js',wa2],['sw.js',sw]]){
...
if(!install.includes("s.src='./wa2-design-pass.js'"))fail('mobile-install-fix.js must load wa2-design-pass.js');
...
for(const marker of ['WHITE ALBUM 2','WINTER STUDY EDITION','wa2AlbumArt','wa2Piano','wa2TrackRow','wa2KnowledgeCard','wa2SnowField','prefers-reduced-motion']){
  if(!wa2.includes(marker))fail(`wa2-design-pass.js missing visual marker: ${marker}`);
}
...
const requiredCache=[...localScripts,'./mobile-tools-drawer.js','./theme-pass.js','./wa2-design-pass.js'];
