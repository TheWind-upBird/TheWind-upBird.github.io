...
for (const file of files) {
  ...
  // Isolate top-level const/let names while preserving writes to window.*
...
ps.forEach((p, idx) => {
  ...
  if (idx > 0) {
    ...
    else for (const k of ['title','example','observe','question','answer']) if (!nonEmpty(intro[k])) fail(`#${pos} ${p.slug} intro missing ${k}`);
    ...
    if (cfg.fill?.targets) {
      for (const t of cfg.fill.targets) {
        if (!patternLines.includes(t)) fail(`#${pos} ${p.slug} fill target not found in pattern: ${t}`);
      }
    }
