...
function showHelp(mode='generic'){
  ...
  else if(mode==='blocked')body=`<p><b>浏览器已经接受了安装请求，但系统没有创建图标。</b>这通常不是网页问题，而是手机启动器/权限拦截。</p><div class="installSteps"><span>设置 → 应用 → Chrome（或 Edge）→ 权限 / 其他权限 → 开启<b>“创建桌面快捷方式”</b>。</span><span>桌面双指捏合 → 桌面设置 → 关闭<b>“锁定布局”</b>。</span><span>然后彻底关闭浏览器，再重新打开 SolveShift 重试。</span></div>`;
  ...
  box.innerHTML=`<div class="installHelpCard"><div class="installHelpHead"><b>添加 SolveShift 到桌面</b><button class="round" id="closeInstallHelp">×</button></div>${body}<button class="primary" id="closeInstallHelp2">知道了</button></div>`;
...
function mount(){
  ...
  const btn=document.createElement('button');btn.id='mobileInstallBtn';btn.className='ghost';btn.type='button';btn.innerHTML='<span aria-hidden="true">⇩</span>';btn.title='添加到桌面';btn.setAttribute('aria-label','添加 SolveShift 到桌面');btn.addEventListener('click',install);
