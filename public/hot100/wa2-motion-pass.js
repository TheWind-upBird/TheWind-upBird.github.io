...
function readPref(){
  try{const v=localStorage.getItem(KEY);return ['auto','on','off'].includes(v)?v:'auto'}catch(e){return'auto'}
}
function pref(){return currentPref}
function shouldRun(){return currentPref==='on'||(currentPref==='auto'&&!reduce.matches)}
...
function stampPref(){root.dataset.wa2MotionPref=currentPref}

function hideAllEffects(){
  stopAll();stampPref();
  root.dataset.wa2Motion='off';
  hide(snowLayer());hide(snowField());hide(moonlight());
  document.querySelectorAll('#wa2FallingSnow .wa2Flake').forEach(f=>{
    try{f.getAnimations().forEach(a=>a.cancel())}catch(e){}
    f.style.animation='none';f.style.opacity='0';f.style.transform='none';
  });
  const field=snowField();if(field){try{field.getAnimations().forEach(a=>a.cancel())}catch(e){}field.style.animation='none';field.style.transform='none'}
  const light=moonlight();if(light){try{light.getAnimations().forEach(a=>a.cancel())}catch(e){}light.style.opacity='0'}
  sync();
}

function staticWinter(){
  stopAll();stampPref();root.dataset.wa2Motion='static';
  show(snowLayer());show(snowField());hide(moonlight());
  ...
  document.querySelectorAll('#wa2FallingSnow .wa2Flake').forEach((f,i)=>{
    f.style.animation='none';f.style.opacity=String(.22+(i%5)*.045);
...
function play(){
  stopAll();stampPref();
  if(root.dataset.theme!=='wa2'){
    hide(snowLayer());hide(snowField());hide(moonlight());root.dataset.wa2Motion='inactive';sync();return;
  }
  if(currentPref==='off'){hideAllEffects();return}
  if(currentPref==='auto'&&reduce.matches){staticWinter();return}
...
function setPref(v){
  if(!['auto','on','off'].includes(v))return;
  currentPref=v;stampPref();
  try{localStorage.setItem(KEY,v)}catch(e){}
  if(v==='off')hideAllEffects();else play();
}

function sync(){
  document.querySelectorAll('[data-wa2-motion]').forEach(b=>{
    const active=b.dataset.wa2Motion===currentPref;
    b.classList.toggle('active',active);b.setAttribute('aria-pressed',active?'true':'false');
  });
  const status=document.getElementById('wa2MotionSystem');
  if(status)status.textContent=reduce.matches?'系统当前：减少动画':'系统当前：允许动画';
}
...
function mount(){
  ...
  box.querySelectorAll('[data-wa2-motion]').forEach(b=>b.addEventListener('click',()=>setPref(b.dataset.wa2Motion)));
  sync();
}
...
html[data-theme="wa2"][data-wa2-motion-pref="off"] #wa2FallingSnow,html[data-theme="wa2"][data-wa2-motion-pref="off"] #wa2VisualLayer .wa2SnowField,html[data-theme="wa2"][data-wa2-motion-pref="off"] .wa2MotionLight,html[data-theme="wa2"][data-wa2-motion-pref="off"] .wa2AlbumArt:after{display:none!important}
html[data-theme="wa2"][data-wa2-motion-pref="off"] body{background:linear-gradient(180deg,#eaf2f8 0,#f5f8fb 40%,#edf3f8 100%)!important;background-attachment:fixed!important}
...
window.addEventListener('hot100themechange',()=>setTimeout(play,0));
reduce.addEventListener?.('change',()=>{if(currentPref==='auto')play();sync()});
const mo=new MutationObserver(()=>{
  mount();
  if(root.dataset.theme==='wa2'&&currentPref==='off'){hide(snowLayer());hide(snowField());hide(moonlight());return}
  if(root.dataset.theme==='wa2'&&document.getElementById('wa2FallingSnow')&&animations.length===0&&shouldRun())play();
});
