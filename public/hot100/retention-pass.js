...
function openWeeklyReport(){
  document.getElementById('weeklyReport')?.remove();const previousFocus=document.activeElement,overlay=document.createElement('div');overlay.id='weeklyReport';overlay.className='weeklyReport';overlay.innerHTML=reportMarkup(weeklySummary());document.body.appendChild(overlay);document.body.classList.add('productModalOpen');
  let modalAccess=null;const close=()=>{modalAccess?.release?.();overlay.remove();document.body.classList.remove('productModalOpen');if(!modalAccess)previousFocus?.focus?.()};overlay.querySelectorAll('[data-weekly-close]').forEach(button=>button.addEventListener('click',close));
  overlay.querySelector('#weeklyContinue')?.addEventListener('click',()=>{modalAccess?.release?.(false);modalAccess=null;overlay.remove();document.body.classList.remove('productModalOpen');const task=Policy.todayPlan(Profile.get().dailyMinutes||20)?.tasks?.[0];if(task)Shell.openTask(task)});window.HOT100_ANALYTICS?.track('weekly_report_open',{source:'home'});modalAccess=window.HOT100_ACCESSIBILITY?.activateModal?.(overlay,{onEscape:close,initialFocus:'.weeklyReportCard'});if(!modalAccess){overlay.addEventListener('keydown',event=>{if(event.key==='Escape')close()});overlay.querySelector('.weeklyReportCard')?.focus()}
}
