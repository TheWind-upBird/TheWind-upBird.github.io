...
function updateDynamic(){
  ...
  const date=document.getElementById('wa2LogDate');if(date)date.textContent=localToday();
  const track=document.getElementById('wa2NowTrack');if(track)track.textContent=`${String(n).padStart(2,'0')} · ${p?.title||'Hot100'}`;
  const record=document.getElementById('wa2RecordCount');if(record)record.textContent=`${solved} / ${CURRICULUM.length}`;
  document.querySelectorAll('.wa2AlbumMeta em').forEach(el=>el.textContent=`TRACK ${String(n).padStart(2,'0')} / ${String(CURRICULUM.length).padStart(3,'0')}`);
  document.querySelectorAll('.wa2HeroNote span').forEach(el=>el.textContent=`WINTER NOTE ${String(n).padStart(2,'0')}`);
  document.querySelectorAll('.wa2DeckLabel small').forEach(el=>el.textContent=`TRACK ${String(n).padStart(2,'0')} · STEP BY STEP`);
}
function decorate(){
