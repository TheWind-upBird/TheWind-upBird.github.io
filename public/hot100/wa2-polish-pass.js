...
function addFallingSnow(){
  if(document.getElementById('wa2FallingSnow'))return;
  const snow=document.createElement('div');snow.id='wa2FallingSnow';snow.setAttribute('aria-hidden','true');
  const glyphs=['❄','✦','•'];
  for(let i=0;i<32;i++){
    const f=document.createElement('span');f.className='wa2Flake';f.textContent=glyphs[i%glyphs.length];
    const x=(i*37+13)%101,drift=((i*29)%70)-35,dur=10+(i%8)*1.45,delay=-(i*2.17%dur),size=7+(i%6)*1.55,op=.18+(i%5)*.055;
    f.style.cssText=`--x:${x}%;--drift:${drift}px;--dur:${dur}s;--delay:${delay}s;--size:${size}px;--op:${op}`;
    snow.appendChild(f);
  }
...
function decorate(){
  addFallingSnow();addStatusStrip();addPageLabels();addCornerRail();updateDynamic();
  document.body.classList.toggle('wa2Polished',root.dataset.theme==='wa2');
}
window.addEventListener('hot100themechange',decorate);
const mo=new MutationObserver(()=>{if(root.dataset.theme==='wa2')requestAnimationFrame(decorate)});mo.observe(document.body,{childList:true,subtree:true});
decorate();

const style=document.createElement('style');style.id='wa2PolishStyles';style.textContent=`
.wa2StatusStrip,.wa2PageLabel,#wa2CornerRail,#wa2FallingSnow{display:none}
...
html[data-theme="wa2"] #wa2FallingSnow{display:block;position:fixed;inset:0;overflow:hidden;pointer-events:none;z-index:80}
html[data-theme="wa2"] .wa2Flake{position:absolute;left:var(--x);top:-10vh;color:#6f96b5;font-size:var(--size);line-height:1;opacity:var(--op);text-shadow:0 0 7px rgba(255,255,255,.92);animation:wa2VisibleFall var(--dur) linear infinite;animation-delay:var(--delay);will-change:transform}
@keyframes wa2VisibleFall{0%{transform:translate3d(0,-10vh,0) rotate(0deg)}50%{transform:translate3d(calc(var(--drift) * .45),52vh,0) rotate(110deg)}100%{transform:translate3d(var(--drift),112vh,0) rotate(230deg)}}
html[data-theme="wa2"] #wa2VisualLayer{z-index:0!important;opacity:.78}
html[data-theme="wa2"] .app,html[data-theme="wa2"] .mobileBar{position:relative;z-index:1}
...
html[data-theme="wa2"] .wa2AlbumArt:after{content:'';position:absolute;z-index:4;inset:-35% -70%;pointer-events:none;background:linear-gradient(112deg,transparent 42%,rgba(224,242,255,.13) 49%,rgba(255,255,255,.24) 51%,rgba(224,242,255,.1) 54%,transparent 61%);transform:translateX(-28%);animation:wa2MoonSweep 8.5s ease-in-out infinite}
@keyframes wa2MoonSweep{0%,20%{transform:translateX(-30%)}65%,100%{transform:translateX(31%)}}
...
html[data-theme="wa2"] .wa2Piano{display:none!important}
html[data-theme="wa2"] .wa2AlbumEdge{z-index:5}
...
html[data-theme="wa2"] .problemRow:hover{background:rgba(255,255,255,.86)!important;transform:translateX(4px)}
html[data-theme="wa2"] .problemRow b{font-family:var(--wa2-serif);font-weight:600;color:#244059}
html[data-theme="wa2"] .problemList{border-top:1px solid #cfdfeb;border-bottom:1px solid #cfdfeb}
html[data-theme="wa2"] .problemList>.problemRow+ .problemRow{border-top:1px solid #dbe7ef!important}

html[data-theme="wa2"] .knowledgeGrid{gap:12px!important}
html[data-theme="wa2"] .wa2KnowledgeCard h3{font-family:var(--wa2-serif);font-weight:600;color:#29465f}
html[data-theme="wa2"] .wa2KnowledgeCard{border-radius:14px!important;transition:transform .18s ease,box-shadow .18s ease}
html[data-theme="wa2"] .wa2KnowledgeCard:hover{transform:translateY(-2px);box-shadow:0 18px 38px rgba(52,84,111,.11)!important}

html[data-theme="wa2"] .wa2StudyCard .cardType,html[data-theme="wa2"] .wa2StudyCard h2,html[data-theme="wa2"] .wa2StudyCard h3{font-family:var(--wa2-serif)}
html[data-theme="wa2"] .wa2StudyCard{box-shadow:0 20px 46px rgba(45,77,105,.09)!important}
html[data-theme="wa2"] .wa2StudyCard:before{width:4px!important}
html[data-theme="wa2"] .deckHead{border-bottom:0}
html[data-theme="wa2"] .deckHead>div:first-child>b{font-family:var(--wa2-serif);font-size:17px;color:#29465f}

html[data-theme="wa2"] #wa2CornerRail{display:flex;position:fixed;right:10px;top:50%;transform:translateY(-50%) rotate(90deg);transform-origin:center;align-items:center;gap:8px;z-index:3;pointer-events:none;font-size:7px;letter-spacing:.22em;color:rgba(80,116,146,.42)}
#wa2CornerRail i{width:38px;height:1px;background:rgba(100,139,169,.3)}
...
html[data-theme="wa2"] .primary{position:relative;overflow:hidden}
html[data-theme="wa2"] .primary:after{content:'';position:absolute;inset:0;transform:translateX(-120%);background:linear-gradient(100deg,transparent,rgba(255,255,255,.23),transparent);transition:transform .45s ease}
html[data-theme="wa2"] .primary:hover:after{transform:translateX(120%)}

@media(prefers-reduced-motion:reduce){html[data-theme="wa2"] .wa2Flake,html[data-theme="wa2"] .wa2AlbumArt:after{animation:none!important}.wa2Flake{transform:translateY(28vh)!important}}
@media(max-width:820px){
  html[data-theme="wa2"] #page-home>.title{font-size:40px;line-height:1.08}
  html[data-theme="wa2"] .wa2StatusStrip{grid-template-columns:1fr 1px 1.5fr;gap:10px;padding:9px 10px}.wa2StatusStrip>div:last-child,.wa2StatusStrip>i:nth-of-type(2){display:none}
  html[data-theme="wa2"] .wa2AlbumArt{min-height:238px!important}
  html[data-theme="wa2"] #wa2CornerRail{display:none}
  html[data-theme="wa2"] .wa2PageLabel{margin-top:2px}
}
