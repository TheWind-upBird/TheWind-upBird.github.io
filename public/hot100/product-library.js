...
function taskReason(task,profile,isEn){
  const zh={
    'due-review':'复习已到期 · 目标：延迟后独立写出','continue-progress':'接着上次的位置 · 目标：完成当前学习证据','weak-pattern':`${task.patternName||'当前 Pattern'} 是薄弱点 · 目标：减少提示`,'next-problem':'路线下一步 · 目标：建立新 Pattern'
  },en={
    'due-review':'Review is due · Evidence: delayed recall','continue-progress':'Resume where you stopped · Evidence: complete this step','weak-pattern':`${task.patternNameEn||'This pattern'} needs work · Evidence: fewer hints`,'next-problem':'Next on your path · Evidence: learn a new pattern'
  };
  if(task.reason==='next-problem'){
    const diagnostic={foundation:isEn?'Placement: build intuition first · Evidence: Understand':'起点诊断：先建立直觉 · 目标：理解',implementation:isEn?'Placement: start from code completion · Evidence: Reproduce':'起点诊断：从代码补空继续 · 目标：复现','practice-ready':isEn?'Placement: attempt independently · Evidence: Solve':'起点诊断：直接独立编程 · 目标：解出'}[profile.diagnosticBand];
    if(diagnostic)return diagnostic
  }
  return(isEn?en:zh)[task.reason]||(isEn?'Recommended by your current evidence':'根据当前掌握证据推荐')
}
function statusCounts(problems){
  let mastered=0,started=0,due=0;const today=window.HOT100_CALENDAR?.dayKey()||'';
...
function enhanceToday(){
  ...
  taskButtons.forEach((button,j)=>{
    ...
    const recommendedMode=button.dataset.productMode||'learn',action=button.querySelector('em');if(action)action.textContent=`${task.minutes} ${isEn?'min':'分钟'} · ${Modes.label(recommendedMode)} ›`;
    ...
    const why=document.createElement('small');why.className='todayTaskReason';why.textContent=taskReason(task,profile,isEn);wrap.appendChild(why)
  });
  bindModeButtons(box)
}
