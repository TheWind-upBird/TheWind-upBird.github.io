...
function todayPlan(minutes=20){
  ...
  if(current&&!seen.has(current.slug)){
    tasks.push({type:'learn',slug:current.slug,index:currentIndex,minutes:Math.max(7,minutes-tasks.reduce((s,x)=>s+x.minutes,0)),title:current.title});seen.add(current.slug)
  }
  if(!tasks.length&&list[0])tasks.push({type:'learn',slug:list[0].slug,index:0,minutes,title:list[0].title});
...
window.HOT100_LEARNING_POLICY={scaffoldPlan,masteryEvidence,todayPlan,patternId,learnedInPattern,hasMeaningfulExposure,cards:{full:FULL},principle:'user-controlled-modes'};
})();