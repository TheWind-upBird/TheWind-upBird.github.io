...
function plan(stage,support,exposure,reason){return{stage,support,cards:FULL,exposure,reason}}
function scaffoldPlan(p){
  const role=p?.productMeta?.role||'anchor';
  const exposure=learnedInPattern(p);
  if(role==='interview'){
    if(exposure===0)return plan('learn','full',exposure,'interview-without-foundation');
    if(exposure===1)return plan('practice','guided',exposure,'interview-with-guided-support');
    return plan('prove','challenge',exposure,'interview-ready');
  }
  if(role==='transfer'){
    if(exposure===0)return plan('learn','full',exposure,'transfer-without-anchor');
    if(exposure===1)return plan('practice','guided',exposure,'first-transfer');
    return plan('practice','light',exposure,'repeated-transfer');
  }
  if(exposure===0)return plan('learn','full',exposure,'first-pattern-exposure');
  if(exposure===1)return plan('practice','guided',exposure,'second-pattern-exposure');
  if(exposure<=3)return plan('practice','light',exposure,'fading-support');
  return plan('prove','challenge',exposure,'established-pattern');
}
...
function todayPlan(minutes=20){
  ...
  if(current&&!seen.has(current.slug)){
    const policy=scaffoldPlan(current);
    tasks.push({type:policy.stage==='prove'?'prove':policy.stage==='practice'?'practice':'learn',slug:current.slug,index:currentIndex,minutes:Math.max(7,minutes-tasks.reduce((s,x)=>s+x.minutes,0)),title:current.title,stage:policy.stage,support:policy.support});seen.add(current.slug)
  }
  if(!tasks.length&&list[0])tasks.push({type:'learn',slug:list[0].slug,index:0,minutes,title:list[0].title,stage:'learn',support:'full'});
...
window.HOT100_LEARNING_POLICY={scaffoldPlan,masteryEvidence,todayPlan,patternId,learnedInPattern,hasMeaningfulExposure,cards:{full:FULL},principle:'keep-all-eight-cards'};
})();