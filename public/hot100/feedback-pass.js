...
function openFeedback(){
  ...
  let modalAccess=null;const close=()=>{modalAccess?.release?.();overlay.remove();document.body.classList.remove('productModalOpen');if(!modalAccess)previous?.focus?.()},type=overlay.querySelector('#feedbackType'),severity=overlay.querySelector('#feedbackSeverity'),description=overlay.querySelector('#feedbackDescription'),include=overlay.querySelector('#feedbackDiagnostics'),status=overlay.querySelector('#feedbackStatus'),preview=overlay.querySelector('#feedbackDiagnosticPreview');
  const refresh=()=>{preview.textContent=include.checked?diagnosticReport():'未附带诊断信息。'};refresh();include.addEventListener('change',refresh);overlay.querySelectorAll('[data-feedback-close]').forEach(button=>button.addEventListener('click',close));
  ...
  window.HOT100_ANALYTICS?.track('feedback_open',{source:'tools'});modalAccess=window.HOT100_ACCESSIBILITY?.activateModal?.(overlay,{onEscape:close,initialFocus:'.feedbackCard'});if(!modalAccess){overlay.addEventListener('keydown',event=>{if(event.key==='Escape')close()});overlay.querySelector('.feedbackCard').focus()}
}
function mountFeedbackTools(){
  ...
  const danger=panel.querySelector('.dangerGroup');if(danger)danger.insertAdjacentElement('beforebegin',group);else panel.appendChild(group);group.querySelector('#openFeedback').addEventListener('click',()=>{window.HOT100_MOBILE_TOOLS?.close?.(false);document.getElementById('mobileToolsBtn')?.focus?.();openFeedback()})
}
